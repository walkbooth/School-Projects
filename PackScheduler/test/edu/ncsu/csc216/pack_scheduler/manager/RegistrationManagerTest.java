...
/**
 * Tests class RegistrationManager
 * @author Alex Johnson
 * @author James Ritchey
 * @author Connor McCarthy 
 */
public class RegistrationManagerTest {
	/** A valid student records file */
	private final String validTestFile = "test-files/student_records.txt";
	/** A valid student records file */
	private final String validFTestFile = "test-files/faculty_records.txt";
	/** Invalid course records */
	private final String invalidTestFile = "test-files/invalid_student_records.txt";
	/** A valid student records file */
	private final String validCatalog = "test-files/course_records.txt";
	/** Invalid course records */
	private final String invalidCatalog = "test-files/invalid_course_records.txt";


	/** The registration manage being tested */
	...
	/**
	 * Tests method getCourseCatalog, of class Registration Manager
	 */
	@Test
	public void testGetCourseCatalog() {
		// test that getCourseCatalog returns a functioning CourseCatalog
		CourseCatalog catalog = manager.getCourseCatalog();
		assertTrue(catalog instanceof CourseCatalog);
		catalog.loadCoursesFromFile(invalidCatalog);
		assertEquals(0, catalog.getCourseCatalog().length);
		catalog.loadCoursesFromFile(validCatalog);
		assertEquals(8, catalog.getCourseCatalog().length);
	}

	/**
	 * Tests method getStudentDirectory, of class Registration Manager
	 */
	@Test
	public void testGetStudentDirectory() {
		// test that getStudentDirectory returns a functioning StudentDirectory
		StudentDirectory directory = manager.getStudentDirectory();
		assertTrue(directory instanceof StudentDirectory);
		directory.loadStudentsFromFile(invalidTestFile);
		assertEquals(0, directory.getStudentDirectory().length);
		directory.loadStudentsFromFile(validTestFile);
		assertEquals(10, directory.getStudentDirectory().length);
	}

	/**
	 * Tests method Login, of class Registration Manager
	 */
	@Test
	public void testLoginStudent() {
		manager.logout();
		manager.getStudentDirectory().loadStudentsFromFile(validTestFile);
		Student s = manager.getStudentDirectory().getStudentById("daustin");
		assertTrue(manager.login("registrar", "Regi5tr@r"));
		manager.logout();
		assertFalse(manager.login("daustin", "pwlol"));
		assertTrue(manager.login("daustin", "pw"));
		assertEquals(s, manager.getCurrentUser());
	}
	...
	public void testLoginFaculty() {
		manager.logout();
		manager.getFacultyDirectory().loadFacultyFromFile(validFTestFile);
		User u = manager.getFacultyDirectory().getFacultyById("awitt");
		assertTrue(manager.login("registrar", "Regi5tr@r"));
		manager.logout();
		assertFalse(manager.login("awitt", "pwlol"));
		assertTrue(manager.login("awitt", "pw"));
		assertEquals(u, manager.getCurrentUser());
	}

	/**
	 * Tests method logout, of class Registration Manager
	 */
	@Test
	...
	/**
	 * Tests method getCurrentUser, of class Registration Manager
	 */
	@Test
	public void testGetCurrentUser() {
		...
		User testStudent = manager.getCurrentUser();
		
		assertNotNull(testStudent);
		assertEquals("Zahir", testStudent.getFirstName());
		assertEquals("King", testStudent.getLastName());
		assertEquals("zking", testStudent.getId());
		assertEquals("orci.Donec@ametmassaQuisque.com", testStudent.getEmail());
	}
	...
	/**
	 * Tests method addFacultToDirectory, of class RegistrationManager
	 */
	@Test
	public void testAddFacultyToCourse() {
		manager.getStudentDirectory().loadStudentsFromFile(validTestFile);
		manager.getFacultyDirectory().loadFacultyFromFile(validFTestFile);
		manager.getCourseCatalog().loadCoursesFromFile(validCatalog);
		
		Faculty f1 = manager.getFacultyDirectory().getFacultyById("fmeadow");
		Faculty f2 = manager.getFacultyDirectory().getFacultyById("bbrewer");
		Faculty f3 = new Faculty("Jessica", "Schmidt", "jdyoung2", "jdyoung2@ncsu.edu", "pw", 2);
		Course c0 = manager.getCourseCatalog().getCourseFromCatalog("CSC116", "001");
		Course c1 = manager.getCourseCatalog().getCourseFromCatalog("CSC116", "002");
		Course c2 = manager.getCourseCatalog().getCourseFromCatalog("CSC216", "001");
		
		c0.setInstructorId(null);
		c1.setInstructorId(null);
		c2.setInstructorId(null);

	    manager.login("registrar", "Regi5tr@r");
		assertTrue(manager.addFacultyToCourse(c0, f3));
		assertTrue(manager.addFacultyToCourse(c1, f1));
		assertTrue(manager.addFacultyToCourse(c2, f1));
		
		assertFalse(manager.addFacultyToCourse(c2, f2));
    	manager.removeFacultyFromCourse(c0, f3); // prepare for student test
	    ...
	    manager.login("efrost", "pw");
	    try {
	    	manager.addFacultyToCourse(c0, f3);
	    	fail("Student dropping faculty from course allowed");
	    } catch (IllegalArgumentException e) {
			assertTrue(manager.getCurrentUser() instanceof Student);
	    }
	}
	...
	/**
	 * Tests method removeFacultFromDirectory, of class RegistrationManager
	 */
	@Test
	public void testRemoveFacultyFromCourse() {
		...
		Faculty f0 = manager.getFacultyDirectory().getFacultyById("awitt");
		Faculty f1 = manager.getFacultyDirectory().getFacultyById("fmeadow");
		Faculty f2 = manager.getFacultyDirectory().getFacultyById("bbrewer");
		Course c0 = manager.getCourseCatalog().getCourseFromCatalog("CSC116", "001");
		Course c1 = manager.getCourseCatalog().getCourseFromCatalog("CSC116", "002");
	    ...
	    manager.login("registrar", "Regi5tr@r");
		assertTrue(manager.addFacultyToCourse(c0, f0));
		assertTrue(manager.addFacultyToCourse(c1, f1));

		assertTrue(manager.removeFacultyFromCourse(c0, f0));
		assertFalse(manager.removeFacultyFromCourse(c2, f2)); //instructor not in the course	
		manager.logout();
		assertNull(manager.getCurrentUser());
		
	    manager.login("efrost", "pw");
	    try {
	    	manager.removeFacultyFromCourse(c1, f1);
	    	fail("Student removing faculty from course allowed");
	    } catch (IllegalArgumentException e) {
			assertTrue(manager.getCurrentUser() instanceof Student);
	    }
	}
